...
<div class="gallerywp-grid-post-inside">
    ...
    <div class="gallerywp-grid-post-thumbnail">
        <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'gallerywp' ), the_title_attribute( 'echo=0' ) ) ); ?>" class="gallerywp-grid-post-thumbnail-link"><?php the_post_thumbnail(gallerywp_grid_thumb_style(), array('class' => 'gallerywp-grid-post-thumbnail-img')); ?></a>
        <?php gallerywp_grid_postmeta(); ?>
    </div>
    <?php } else { ?>
    <div class="gallerywp-grid-post-thumbnail">
        <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'gallerywp' ), the_title_attribute( 'echo=0' ) ) ); ?>" class="gallerywp-grid-post-thumbnail-link"><img src="<?php echo esc_url( get_template_directory_uri() . '/assets/images/no-image-4-4.jpg' ); ?>" class="gallerywp-grid-post-thumbnail-img"/></a>
        <?php gallerywp_grid_postmeta(); ?>
    </div>
    <?php } ?>
    <?php } else { ?>
